  ...
  return (
    <article className="vacat-card vacat-card-glow group flex min-h-full flex-col overflow-hidden rounded-[24px]">
      <div className="relative aspect-[16/11] overflow-hidden bg-black video-shell">
        {template.previewVideoUrl ? (
          ...
          <Link href={`/templates/${template.id}`} className="block h-full">
            <img src={template.thumbnailUrl} alt={template.title} className="h-full w-full object-cover opacity-90" />
          </Link>
        )}
        <div className="pointer-events-none absolute left-4 top-4 rounded-full border border-white/10 bg-black/55 px-3 py-1 text-xs font-semibold text-lime-200 backdrop-blur">
          {template.id}
        </div>
      ...
      <div className="flex flex-1 flex-col p-5">
        <Link href={`/templates/${template.id}`} className="transition hover:text-lime-200">
