    ...
    <article className="group flex min-h-full flex-col overflow-hidden rounded-[24px] border border-white/[0.08] bg-[#0d0d0d] shadow-2xl shadow-black/20 transition duration-300 hover:-translate-y-1 hover:border-white/[0.18]">
      <Link href={`/templates/${template.id}`} className="block">
        <div className="relative aspect-[16/11] overflow-hidden bg-black video-shell">
          ...
          <div className="absolute left-4 top-4 rounded-full border border-white/10 bg-black/55 px-3 py-1 text-xs font-semibold text-white backdrop-blur">
            {template.id}
          </div>
          <div className="absolute bottom-4 left-4 rounded-full border border-white/10 bg-white/[0.12] px-3 py-1 text-xs text-white backdrop-blur">
