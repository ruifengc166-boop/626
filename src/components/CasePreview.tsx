    ...
    <section className="mx-auto max-w-[1200px] px-6 py-24">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div className="max-w-2xl">
          <p className="mb-3 text-xs font-semibold uppercase tracking-[0.3em] text-white/38">Sample results</p>
          <h2 className="text-3xl font-medium tracking-[-0.035em] text-white md:text-5xl">See the shift.</h2>
        </div>
        <p className="max-w-sm text-sm leading-6 text-white/42">From style preview to product ad direction.</p>
      </div>
            ...
            <div className="rounded-[18px] border border-white/[0.06] bg-[radial-gradient(circle_at_50%_0%,rgba(255,255,255,0.1),rgba(255,255,255,0.025)_45%,rgba(0,0,0,0)_100%)] p-4">
              ...
              <div className="relative overflow-hidden rounded-xl border border-white/[0.08] bg-[#111]">
                {caseItem.afterVideoUrl ? (
                  ...
                  <img src={caseItem.thumbnailUrl} alt={`${caseItem.title} product ad`} className="aspect-[16/10] h-full w-full object-cover" />
                )}
                <div className="absolute left-3 top-3 rounded-full bg-black/60 px-3 py-1 text-[10px] font-medium uppercase tracking-[0.18em] text-white/70 backdrop-blur">
                  Product Ad
                </div>
