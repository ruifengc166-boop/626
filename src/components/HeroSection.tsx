...
const trustLabels = [
  "Fast product ad production",
  "Lower cost than custom shoots",
...
export function HeroSection({ template }: { template: Template }) {
  return (
    <section className="mx-auto grid max-w-[1200px] gap-12 px-6 py-24 md:py-28 lg:grid-cols-[minmax(0,1fr)_420px] lg:items-start">
      <div className="pt-4">
        <p className="mb-5 text-xs font-semibold uppercase tracking-[0.32em] text-white/38">Product Ads. Faster.</p>
        <h1 className="max-w-5xl text-[2.75rem] font-medium leading-[1.05] tracking-[-0.045em] text-white md:text-6xl lg:text-[4.7rem]">
          High-quality product ads without the agency timeline.
        </h1>
        <p className="mt-5 max-w-3xl text-base leading-7 text-white/64 md:text-lg">
          Send your product, logo and selling points. Get a polished short-form ad made for testing on TikTok, Reels, Shorts and paid social.
        </p>
        <div className="mt-8 grid max-w-3xl gap-3 sm:grid-cols-3">
          <Metric value="24-48h" label="first draft option" />
          <Metric value="$299+" label="polished ad delivery" />
          <Metric value="5×" label="variation pack available" />
        </div>
