    ...
    <main className="mx-auto max-w-[1200px] px-6 py-16">
      <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h1 className="text-4xl font-semibold tracking-[-0.04em] text-white md:text-6xl">Cases</h1>
          <p className="mt-4 max-w-2xl text-sm leading-6 text-white/56">
            Manage homepage case previews and internal case notes. Use video URLs when available.
          </p>
        </div>
      ...
      <section className="mt-10 rounded-[1.75rem] border border-white/[0.08] bg-[#0d0d0d] p-5 shadow-2xl shadow-black/20">
      ...
      <section className="mt-10 space-y-5">
        {cases.map((caseItem) => (
          <article key={caseItem.id} className="rounded-[1.75rem] border border-white/[0.08] bg-[#0d0d0d] p-5 shadow-2xl shadow-black/20">
            <div className="grid gap-5 lg:grid-cols-[280px_1fr]">
              ...
              <div>
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    <p className="text-xs uppercase tracking-[0.22em] text-white/36">{caseItem.id}</p>
    ...
    <form action={action} className="mt-5 grid gap-4">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <label className="block">
          <span className="mb-2 block text-sm font-medium text-white/74">Template</span>
          <select name="templateId" defaultValue={caseItem?.templateId || "T001"} className="input">
            {templates.map((template) => <option key={template.id} value={template.id}>{template.id} · {template.title}</option>)}
          </select>
        </label>
        <Input label="Product Category" name="productCategory" defaultValue={caseItem?.productCategory || ""} />
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Input label="Before Asset URL" name="beforeAssetUrl" defaultValue={caseItem?.beforeAssetUrl || ""} placeholder="/assets/thumbnails/t001_thumbnail.jpg" />
        <Input label="After Video URL" name="afterVideoUrl" defaultValue={caseItem?.afterVideoUrl || ""} placeholder="/assets/cases/case_beauty_adaptation.mp4" />
        <Input label="Thumbnail URL" name="thumbnailUrl" defaultValue={caseItem?.thumbnailUrl || ""} placeholder="/assets/thumbnails/case_beauty_thumbnail.jpg" />
      </div>
      ...
      <button type="submit" className="w-fit rounded-full bg-white px-5 py-2.5 text-sm font-medium text-black transition hover:bg-white/85">{submitLabel}</button>
    </form>
  );
}
