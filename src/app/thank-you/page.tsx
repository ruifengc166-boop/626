...
export default async function ThankYouPage({ searchParams }: { searchParams: Promise<{ template?: string; brand?: string }> }) {
    ...
    <main className="mx-auto flex min-h-[70vh] max-w-3xl flex-col items-center justify-center px-6 text-center">
      ...
      {template || brand ? (
        <div className="mt-8 rounded-[1.5rem] border border-white/[0.08] bg-white/[0.04] px-5 py-4 text-sm text-white/62">
          {template ? <p>Selected style: <span className="text-white">{template}</span></p> : null}
